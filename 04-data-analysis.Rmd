...
test <- df_stigma_long %>%
  select(gen_sds_shortterm, gen_infektion_v, gen_alter_v, gen_beruf_v, gen_sex_v, gen_ethnie_v, id) %>% 
  drop_na()

## 1. Build linear model 

ranin_test <-  lmer(gen_sds_shortterm ~ gen_infektion_v + gen_alter_v + gen_beruf_v + gen_sex_v + gen_ethnie_v + gen_infektion_v*gen_beruf_v + (1 | id), data = test)

## 2. Plot Model

# 2.1 Plot predicted Values with plot_model

plot_model(ranin_test, type = "pred", terms = c("gen_infektion_v", "gen_beruf_v"))


# 2.2 Plot predicted Effects with predictorEffects

plot(predictorEffect("gen_infektion_v", ranin_test), lines=list(multiline=TRUE))
plot(predictorEffects(ranin_test, ~ gen_beruf_v + gen_infektion_v), lines=list(multiline=TRUE))


# 2.3 Add predictions & Plot Regression Line

pred.int <- predict(ranin_test, interval = "prediction")
mydata <- cbind(test, pred.int)

ggplot(mydata, aes(gen_sds_shortterm, pred.int, col = gen_beruf_v)) +
  #geom_point() +
