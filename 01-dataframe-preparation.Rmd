...
```{r Datensatz reinladen, include = FALSE}
...
# Keine Missings vorhanden, da mit Quotierung gearbeitet wurde und man nicht überspringen konnte. 
...
df_nolab <- df_nolab%>%
  mutate(gen_bund = ifelse(SD300 == c(4, 8, 13, 14, 16), 0, 1))
...
### Berufsausbildung SD500* - gen_beruf
...
### Staatsangehörigkeit SD600 - gen_staat
...
### Geburtsort Eltern SD700

# 1 = Ja (Ausland)
# 2 = Nein (Nicht im Ausland)
# 99 = Missing
...
# Deutsche Staatsangehörigkeit X Geburtsort Eltern Missing = 5 Fälle --> unklar
# Staatsangehörigkeit Missing X Geburtsort Eltern Außerhalb = 2 Fälle --> aufjedenfall migrationshintergund
# Staatsangehörigkeit Missing X Geburtsort Eltern Deutschland = 13 Fälle --> unklar
# Staatsangehörigkeit Missing X Geburtsort Eltern Missing = 1 Fälle --> unklar
...
### Einstellung der Lage EC19100 - EC19400

## Einstellung: Eigene Finanzielle Situation - EC19100

df_nolab$EC19100[df_nolab$EC19100 == 0] <- 99
count(df_nolab, EC19100)

## Einstellung: Eigene Gesundheit - EC19200

df_nolab$EC19200[df_nolab$EC19200 == 0] <- 99
count(df_nolab, EC19200)

## Einstellung: Gesundheit Angehöriger - EC19300

df_nolab$EC19300[df_nolab$EC19300 == 0] <- 99
count(df_nolab, EC19300)

## Einstellung: Allgemeine Wirtschaftliche Lage - EC19400
...
### Einstellung Maßnahmen MC19100_1 - MC19100_9

## Einstellung Maßnahmen: Schließung von Schulen und Kindergärten - MC19100_1
...
df_nolab <- df_nolab %>%
  mutate(gen_impfung = ifelse((IC19200 == 1), 1,
                              ifelse((IC19200 == 2), 1,
                                     ifelse((IC19200 == 3) & (IC19300 == 1), 2,
                                            ifelse((IC19200 == 3) & (IC19300 == 2), 2,
...
### Politik - Originalvariable - PE100
...
### Durchführung Umfrage - completed
...
## Second Index Variable - including SDS100 & SDS200 questions
...
## Third Index Variable - including SDS300, SDS400, SDS500 & SDS600 questions
...
# I am going to do 3 Index Variables.
#   1. One including all of the 4 questions
#   2. One including only the ones regarding health
#      -->  According to the factor analysis, being health question were answered similarly, which is why they should create a index variable
#   3. One including only the ones regarding the financial situation
#      -->  According to the factor analysis, these two question seemed to be answered similarly, which is why they should create a index variable

## First Index Variable - including all 4 questions
...
## Second Index Variable - including EC19200 + EC19300 questions

df_stigma_long <- df_stigma_long %>%
  mutate(gen_situation_gesund = (EC19200 + EC19300)/2) %>%
  mutate_if(is.numeric, ~round(., 2))

# df_stigma_long$gen_situation_gesund


## Third Index Variable - including EC19100 + EC19400 questions

df_stigma_long <- df_stigma_long %>%
  mutate(gen_situation_finanzen = (EC19100 + EC19400)/2) %>%
  mutate_if(is.numeric, ~round(., 2))

# +üdf_stigma_long$gen_situation_finanzen
