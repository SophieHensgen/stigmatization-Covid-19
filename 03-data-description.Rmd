...
# soon subset of the data - only important variables should be included, such as age, sex etc.
df_stigma_subset <- df_stigma_long %>%
  select(gen_sds_shortterm, gen_sds_longterm, gen_infektion_v, gen_ethnie_v , gen_beruf_v, gen_alter_v, SD100, gen_alter, gen_bund, gen_schule, gen_schule_q)

# Frage:
#   Welche Werte sollten am besten abgefragt werden? 
#   -   Min und Max
#   -   Mean
#   -   N
#   -   Standard Deviation

stargazer(data.frame(df_stigma_subset), type = "latex", summary = TRUE,summary.stat = c("n", "min", "max", "mean", "median", "sd"),  out = "summary-1.tex")


d <- (as.data.frame(table(df_stigma_subset$SD100)) %>% rename(Count=1,Freq=2) %>% mutate(Perc=100*Freq/sum(Freq)))
f <- (as.data.frame(table(df_stigma_subset$gen_alter)) %>% rename(Count=1,Freq=2) %>% mutate(Perc=100*Freq/sum(Freq)))
e <- (as.data.frame(table(df_stigma_subset$gen_bund)) %>% rename(Count=1,Freq=2) %>% mutate(Perc=100*Freq/sum(Freq)))
g <- (as.data.frame(table(df_stigma_subset$gen_schule_q)) %>% rename(Count=1,Freq=2) %>% mutate(Perc=100*Freq/sum(Freq)))

df <- bind_rows(d, f, e, g)

stargazer(data.frame(df), type = "latex", 
       summary = FALSE,
...
# The index variable including all 4 Variables has a similar look as a normal distributon, however, 1 is much higher than the other values.
...
# After inspection of the graph is clear, that the variables only load on one factor.
...
# Interpretation:
   #  In the graph we see, that EC19100 and EC10400 have similar values. EC19200 and EC19300 have similar values. Which makes sense, because 100 and 400 ask after
   #  financial situations and the others after health. In general, they have all similar values.
...
# Es scheint kein Vignetten Effekt vorzuliegen, anhand der descriptiven Statistik
```

```{r Boxplots Alter}

### Boxplot to look into differences 
...
```{r Kontrollvariable Geschlecht Vignette}
...
```{r Kontrollvariable Einstellung derzeitige Lage}

## Einstellung derzeitige Lage
...
## Einstellung derzeitige Lage - Gesundheit

ggplot(df_stigma_long, aes(x = gen_situation_gesund, y = gen_sds,  alpha = 0.5)) +
  geom_jitter()  +
  facet_wrap(~gen_infektion_v)

ggplot(df_stigma_long, aes(x = gen_situation_gesund, y = gen_sds_longterm,  alpha = 0.5)) +
  geom_jitter()  +
  facet_wrap(~gen_infektion_v)

ggplot(df_stigma_long, aes(x = gen_situation_gesund, y = gen_sds_shortterm,  alpha = 0.5)) +
  geom_jitter()  +
  facet_wrap(~gen_infektion_v)


## Einstellung derzeitige Lage - Finanzen

ggplot(df_stigma_long, aes(x = gen_situation_finanzen, y = gen_sds,  alpha = 0.5)) +
  geom_jitter()  +
  facet_wrap(~gen_infektion_v)

ggplot(df_stigma_long, aes(x = gen_situation_finanzen, y = gen_sds_longterm,  alpha = 0.5)) +
  geom_jitter()  +
  facet_wrap(~gen_infektion_v)

ggplot(df_stigma_long, aes(x = gen_situation_finanzen, y = gen_sds_shortterm,  alpha = 0.5)) +
  geom_jitter()  +
  facet_wrap(~gen_infektion_v)
```


```{r Kontrollvariable Politische Einstellung}
